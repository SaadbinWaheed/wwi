import "../managementSection.css";

function Management() {
  return (
    <div
      style={{ margin: "-4.1rem 0 0 0" }}
      class="mb-16 p-10 px-12 bg-gray-100 pb-16"
    >
      <dh-component>
        <div class="container flex sm:justify-center md:justify-center lg:justify-start xl:justify-start my-auto mx-auto pt-10 bg-gray-100">
          <div className="p-5">
            <h1 class="xl:text-5xl text-5xl text-start colors pl-5 font-bold pb-6 sm:w-4/6 w-5/6">
              Management{" "}
            </h1>
          </div>
        </div>
        <div class="w-full bg-gray-100 px-10 pt- pb-10">
          <div class="container mx-auto center px-7 py-2 pt-20 bg-gray-100">
            <div
              role="list"
              aria-label="Behind the scenes People "
              class="lg:flex md:flex sm:flex items-center xl:justify-between flex-wrap md:justify-around sm:justify-around lg:justify-around"
            >
              <div
                role="listitem"
                class="xl:w-1/3 sm:w-3/4 md:w-2/5 relative mt-16 mb-32 sm:mb-24 xl:max-w-sm lg:w-2/5"
              >
                <div class="rounded overflow-hidden">
                  <div class="absolute -mt-20 w-full flex justify-center">
                    <div class="h-32 w-32">
                      <img
                        src="https://cdn.tuk.dev/assets/photo-1570211776045-af3a51026f4a.jfif"
                        alt="Display Picture of Charles Keith"
                        role="img"
                        class="object-cover h-full w-full shadow-md"
                      />
                    </div>
                  </div>
                  <div class="px-6 mt-16">
                    <h1 class="font-bold text-3xl colors text-center mb-1">
                      Aarti Misal
                    </h1>
                    <p class="text-gray-800 text-sm text-center">
                      Research Lead Access to Seeds Index
                    </p>
                  </div>
                </div>
              </div>

              <div
                role="listitem"
                class="xl:w-1/3 sm:w-3/4 md:w-2/5 relative mt-16 mb-32 sm:mb-24 xl:max-w-sm lg:w-2/5"
              >
                <div class="rounded overflow-hidden">
                  <div class="absolute -mt-20 w-full flex justify-center">
                    <div class="h-32 w-32">
                      <img
                        src="https://cdn.tuk.dev/assets/photo-1570211776045-af3a51026f4a.jfif"
                        alt="Display Picture of Charles Keith"
                        role="img"
                        class="object-cover h-full w-full shadow-md"
                      />
                    </div>
                  </div>
                  <div class="px-6 mt-16">
                    <h1 class="font-bold text-3xl colors text-center mb-1">
                      Alice Ingabire
                    </h1>
                    <p class="mt-3 text-gray-800 text-sm text-center">
                      Access to Seeds Index Lead
                    </p>
                  </div>
                </div>
              </div>

              <div
                role="listitem"
                class="xl:w-1/3 sm:w-3/4 md:w-2/5 relative mt-16 mb-32 sm:mb-24 xl:max-w-sm lg:w-2/5"
              >
                <div class="rounded overflow-hidden">
                  <div class="absolute -mt-20 w-full flex justify-center">
                    <div class="h-32 w-32">
                      <img
                        src="https://cdn.tuk.dev/assets/photo-1570211776045-af3a51026f4a.jfif"
                        alt="Display Picture of Charles Keith"
                        role="img"
                        class="object-cover h-full w-full shadow-md"
                      />
                    </div>
                  </div>
                  <div class="px-6 mt-16">
                    <h1 class="font-bold text-3xl colors text-center mb-1">
                      Charlotte Reeves
                    </h1>
                    <p class="mt-3 text-gray-800 text-sm text-center">
                      Engagemennt Manager
                    </p>
                  </div>
                </div>
              </div>

              <div
                role="listitem"
                class="xl:w-1/3 sm:w-3/4 md:w-2/5 relative mt-16 mb-32 sm:mb-24 xl:max-w-sm lg:w-2/5"
              >
                <div class="rounded overflow-hidden">
                  <div class="absolute -mt-20 w-full flex justify-center">
                    <div class="h-32 w-32">
                      <img
                        src="https://cdn.tuk.dev/assets/photo-1570211776045-af3a51026f4a.jfif"
                        alt="Display Picture of Charles Keith"
                        role="img"
                        class="object-cover h-full w-full shadow-md"
                      />
                    </div>
                  </div>
                  <div class="px-6 mt-16">
                    <h1 class="font-bold text-3xl colors text-center mb-1">
                      Gustaf Renman
                    </h1>
                    <p class="mt-3 text-gray-800 text-sm text-center">
                      Research Analyst
                    </p>
                  </div>
                </div>
              </div>

              <div
                role="listitem"
                class="xl:w-1/3 sm:w-3/4 md:w-2/5 relative mt-16 mb-32 sm:mb-24 xl:max-w-sm lg:w-2/5"
              >
                <div class="rounded overflow-hidden">
                  <div class="absolute -mt-20 w-full flex justify-center">
                    <div class="h-32 w-32">
                      <img
                        src="https://cdn.tuk.dev/assets/photo-1570211776045-af3a51026f4a.jfif"
                        alt="Display Picture of Charles Keith"
                        role="img"
                        class="object-cover h-full w-full shadow-md"
                      />
                    </div>
                  </div>
                  <div class="px-6 mt-16">
                    <h1 class="font-bold text-3xl colors text-center mb-1">
                      Joachim Roth
                    </h1>
                    <p class="mt-3 text-gray-800 text-sm text-center">
                      Climate Change Lead
                    </p>
                  </div>
                </div>
              </div>

              <div
                role="listitem"
                class="xl:w-1/3 sm:w-3/4 md:w-2/5 relative mt-16 mb-32 sm:mb-24 xl:max-w-sm lg:w-2/5"
              >
                <div class="rounded overflow-hidden">
                  <div class="absolute -mt-20 w-full flex justify-center">
                    <div class="h-32 w-32">
                      <img
                        src="https://cdn.tuk.dev/assets/photo-1570211776045-af3a51026f4a.jfif"
                        alt="Display Picture of Charles Keith"
                        role="img"
                        class="object-cover h-full w-full shadow-md"
                      />
                    </div>
                  </div>
                  <div class="px-6 mt-16">
                    <h1 class="font-bold text-3xl colors text-center mb-1">
                      Kidan Araya
                    </h1>
                    <p class="mt-3 text-gray-800 text-sm text-center">
                      Communications & Content Lead
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </dh-component>
    </div>
  );
}

export default Management;
